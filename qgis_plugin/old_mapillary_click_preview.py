...
def _build_pano_filter_expr(pano_only):
    """QGIS expression that filters coverage features to only panoramic images."""
    if pano_only:
        return '"is_pano" = true'
    return ''
...
def _fetch_osm_buildings_layer(bounds):
    ...
    layer = QgsVectorLayer('Polygon?crs=EPSG:4326', 'OSM Buildings', 'memory')
    prov = layer.dataProvider()

    features = []
    for el in data.get('elements', []):
        geom = el.get('geometry')
        if not geom or el.get('type') not in ('way', 'relation'):
            continue
...
class MapillaryClickPreviewPlugin:
    ...
    def initGui(self):
        ...
        self.add_buildings_action = QAction('Load OSM Buildings (Overpass) for Current View', self.iface.mainWindow())
    ...
    def _apply_year_filter_to_existing_layers(self):
        ...
        if enabled:
            from_year = s.value('mapillary/year_filter_from', MAPILLARY_LAUNCH_YEAR, type=int)
            to_year = s.value('mapillary/year_filter_to', datetime.now().year, type=int)
            clauses.append(f'({_build_year_filter_expr(from_year, to_year)})')
        if pano_only:
            clauses.append('("is_pano" = true)')
        ...
        for layer in QgsProject.instance().mapLayers().values():
            ...
            applicable = [c for c in clauses if layer.fields().lookupField('is_pano') != -1 or 'is_pano' not in c]
            # simpler: only apply is_pano clause on layers that actually have that field
            subset_parts = []
            if enabled and layer.fields().lookupField('captured_at') != -1:
                subset_parts.append(f'({_build_year_filter_expr(from_year, to_year)})')
    ...
    def _load_buildings(self):
        bounds = self._get_active_aoi_bounds()
        if not bounds:
            canvas = self.iface.mapCanvas()
            crs_src = canvas.mapSettings().destinationCrs()
            xform = QgsCoordinateTransform(crs_src, QgsCoordinateReferenceSystem(4326), QgsProject.instance())
            ex = canvas.extent()
